...
- Name: responsive-ui.spec.js >> 반응형 UI 검증 (FE-12) >> 모바일(375px)에서 할일 추가 버튼이 터치하기 쉬운 위치에 있다
- Location: e2e\responsive-ui.spec.js:80:3
...
Locator: getByRole('button', { name: /새로운 할일/i })
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: /새로운 할일/i })
...
> 89  |     await expect(addButton).toBeVisible({ timeout: 10000 })
  ...
  162 |       expect(buttonBox.height).toBeGreaterThanOrEqual(44)
  163 |     }
  164 |   })
  165 | })
```